...
folder_path = 'C:\Users\User\Desktop\IVC_labs_starting_point\training_images';
...
for scaleIdx = 1 : numel(scales)
    ...
    for i=1:size(img_cell,2)
        [DC{i},AC{i}]=IntraEncode_DPCM(img_cell{i}, qScale, 63);
        % min_DC(i)=min(DC{i});
        % max_DC(i)=max(DC{i});
        % min_AC(i)=min(AC{i});
        % max_AC(i)=max(AC{i});
    ...
    % upper_boundDC = max([max(max_DC) upper_boundDC])%2000
    pmfDC=0;
    pmfAC=0;
    for i=1:size(img_cell,2)
        cur_pmfDC=stats_marg(DC{i},lower_boundDC:upper_boundDC);
        pmfDC=pmfDC+cur_pmfDC;
        cur_pmfAC=stats_marg(AC{i},lower_bound:upper_bound);
        pmfAC=pmfAC+cur_pmfAC;
    end
    pmfDC=pmfDC/sum(pmfDC);
    pmfAC=pmfAC/sum(pmfAC);
    [BinaryTreeDC, HuffCodeDC, BinCodeDC, CodelengthsDC] = buildHuffman(pmfDC);
    [BinaryTreeAC, HuffCodeAC, BinCodeAC, CodelengthsAC] = buildHuffman(pmfAC);
    num=num2str(qScale);
    num = strrep(num, '.', '');
    filename=append('Huffman',num);
    filename=append(filename,'.mat');
    filepath=append('huffman_tables/Chapter5DCACEOB6000/',filename);
    save(filepath,'BinaryTreeDC','HuffCodeDC','BinCodeDC','CodelengthsDC','BinaryTreeAC','HuffCodeAC','BinCodeAC','CodelengthsAC')
end
    ...
    for i = 1 : numBlocksY
        for j = 1 : numBlocksX
            ...
            while halfSearchRange >= 1
                ...
                if (motionVectorItteration(1)==0 && motionVectorItteration(2)==0) || motionVector(1)==range-1 || motionVector(2)==range-1
